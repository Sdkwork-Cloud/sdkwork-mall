...
Consume federated commerce through `@sdkwork/clawrouter-app-sdk/domains` / `@sdkwork/clawrouter-backend-sdk/domains`, `@sdkwork/mall-commerce-service`, and T1 domain SDKs (`@sdkwork/shop-app-sdk`, `@sdkwork/order-app-sdk`, `@sdkwork/payment-app-sdk`, …). No raw HTTP. No local SDK forks. Retired `@sdkwork/commerce-*` and `@sdkwork/clawrouter-*-domain-transport-sdk` packages must not be reintroduced.
