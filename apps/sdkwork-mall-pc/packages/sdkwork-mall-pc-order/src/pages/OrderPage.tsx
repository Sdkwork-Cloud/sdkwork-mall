...
function resolveStatusTone(status: string) {
  ...
  if (status === "pending-shipment" || status === "pending-receipt") {
...
function SdkworkOrderPageContent({
  ...
  const heroHighlights = [
    ...
    },
    {
      ...
      value: state.dashboard.statistics.pendingShipment,
    },
    {
      icon: PackageCheck,
      label: copy.stats.pendingReceipt,
      tone: "accent" as const,
      value: state.dashboard.statistics.pendingReceipt,
    },
    {
      icon: BadgeCheck,
                    ...
                    {order.status === "pending-payment" ? (
                      <>
                        <Button
                          ...
                          onClick={() => void controller.payOrder({ orderId: order.id })}
                          type="button"
                        >
                          去支付
                        </Button>
                        <Button
                          disabled={state.isMutating}
                          onClick={() => void controller.cancelOrder({ orderId: order.id })}
                          type="button"
                          variant="ghost"
                        >
                          取消订单
                    ...
                    {order.status === "paid" || order.status === "completed" ? (
                      <>
                        {order.status === "completed" ? (
                          <Link to={`/buyer/reviews?orderId=${encodeURIComponent(order.id)}`}>
                            <Button type="button" variant="outline">评价晒单</Button>
                          </Link>
                        ) : null}
                        <Link to={`/buyer/after-sales?orderId=${encodeURIComponent(order.id)}`}>
                          <Button type="button" variant="ghost">申请售后</Button>
                        </Link>
                      </>
                    ) : null}
                    {order.status === "refunding" || order.status === "refunded" ? (
                      <Link to={`/buyer/after-sales?orderId=${encodeURIComponent(order.id)}`}>
                        <Button type="button" variant="outline">售后详情</Button>
                      </Link>
                    ) : null}
                  </div>
                </article>
